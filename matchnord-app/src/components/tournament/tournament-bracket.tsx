...
export function TournamentBracket({
  ...
  const sortedRounds = Object.keys(matchesByRound).sort((a, b) => {
    const aIndex = roundOrder.findIndex((round) =>
      a.toLowerCase().includes(round.toLowerCase())
    );
    const bIndex = roundOrder.findIndex((round) =>
      b.toLowerCase().includes(round.toLowerCase())
    );
              ...
              <div key={match.id} className="rounded-lg border p-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-xl font-bold ml-2 flex-shrink-0">
                      {match.homeScore ?? "-"}
                    </div>
                  </div>
