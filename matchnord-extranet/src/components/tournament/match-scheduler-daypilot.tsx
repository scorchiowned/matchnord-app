...
export function MatchSchedulerDayPilot({
  ...
  // Helper function to check if a match matches current filters
  const matchPassesFilters = useCallback(
    (match: Match): boolean => {
      // Apply filters
      if (
  ...
  // Two types: Primary (filtered, interactive) and Context (greyed out, visual only)
  const events = useMemo(() => {
    ...
    scheduledMatches
      ...
      .forEach((match) => {
        const passesFilters = matchPassesFilters(match);

        // Create event based on whether it passes filters
        ...
        if (passesFilters) {
          // PRIMARY EVENT - Fully interactive, detailed info
          ...
          // CONTEXT EVENT - Grey, non-interactive, minimal info (match number only)
          allEvents.push({
            id: `context-${match.id}`, // Different ID to distinguish from primary events
            text: match.matchNumber || 'Match',
            start: start,
            end: end,
            resource: match.pitch!.id,
            backColor: '#e2e8f0', // Light grey
            borderColor: '#cbd5e1',
            fontColor: '#64748b',
            moveDisabled: true, // Cannot drag
            resizeDisabled: true, // Cannot resize
            clickDisabled: false, // Allow click to show tooltip
            html: `
              <div style="padding: 4px; font-size: 10px; line-height: 1.3; display: flex; flex-direction: column; height: 100%; justify-content: center; align-items: center; color: #64748b; opacity: 0.7;">
                ${match.matchNumber ? `<div style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">${match.matchNumber}</div>` : `<div style="font-size: 9px;">Match</div>`}
              </div>
            `,
            toolTip: `
              <div style="padding: 12px; min-width: 200px;">
                <div style="font-size: 12px; font-weight: bold; margin-bottom: 8px; color: #64748b;">Outside Current Filter</div>
                ${match.matchNumber ? `<div style="font-size: 14px; font-weight: bold; margin-bottom: 8px; color: #64748b;">Match ${match.matchNumber}</div>` : ''}
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                  ${match.homeTeam.logo ? `<img src="${match.homeTeam.logo}" alt="" style="width: 24px; height: 24px; object-fit: contain;" />` : ''}
                  <span style="font-weight: bold;">${match.homeTeam.name}</span>
                </div>
                ...
                <div style="border-top: 1px solid #e5e7eb; padding-top: 8px; font-size: 13px;">
                  ...
                  <div style="margin-top: 8px; padding: 8px; background: #fef3c7; border-radius: 4px; font-size: 12px; color: #92400e;">
                    ⚠️ This match is outside your current filter selection. To edit, adjust your filters.
                  </div>
                </div>
              </div>
            `,
    ...
    return allEvents;
  }, [scheduledMatches, divisions, isDateInRange, matchPassesFilters]);

  const handleEventMove = useCallback(
    async (args: any) => {
      const eventId = args.e.id();

      // Prevent moving context events (greyed out matches)
      if (eventId.startsWith('context-')) {
        toast.error(
          'Cannot move matches outside current filter. Please adjust filters first.'
        );
        args.preventDefault();
        return;
      }

      const match = scheduledMatches.find((m) => m.id === eventId);
      if (!match) {
        args.preventDefault();
        return;
      }
  ...
  const handleEventResize = useCallback(
    async (args: any) => {
      ...
      // Prevent resizing context events (greyed out matches)
      if (eventId.startsWith('context-')) {
        toast.error(
          'Cannot resize matches outside current filter. Please adjust filters first.'
        );
        args.preventDefault();
        return;
      }
  ...
  const calendarConfig = useMemo(() => {
    ...
    return {
      ...
      onEventClick: (args: any) => {
        ...
        // Prevent editing context events (greyed out matches)
        if (eventId.startsWith('context-')) {
          // Just show tooltip, don't allow editing
          if (args.originalEvent && args.originalEvent.detail === 2) {
            toast.info(
              'This match is outside your current filter. Adjust filters to edit.'
            );
          }
          return;
        }
